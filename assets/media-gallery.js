  ...
  customElements.define(
    ...
    class MediaGallery extends HTMLElement {
      constructor() {
        ...
        this.mql = window.matchMedia("(min-width: 749px)");
      ...
      onSlideChanged(event) {
        // Attempt to get currentElement from event.detail
        let currentElement = event.detail?.currentElement;

        // Fallback: If currentElement is undefined, look for the active slide
        if (!currentElement) {
          currentElement = this.elements.viewer.querySelector(".is-active");
        }

        if (currentElement && currentElement.dataset?.mediaId) {
